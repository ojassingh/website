...
import { Uniform, Vector2, Texture, WebGLRenderTarget, WebGLRenderer } from "three"
...
class AsciiEffectImpl extends Effect {
  ...
  update(renderer: WebGLRenderer, inputBuffer: WebGLRenderTarget<Texture>, deltaTime?: number) {
    const targetFPSUniform = this.uniforms.get("targetFPS")
    const targetFPS = (targetFPSUniform?.value as number) ?? 0
